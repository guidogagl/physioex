...
Self-contained wrapper: imports from transformers only, no EEGBenchmarks.
...
class REVESleepNet(FoundationModelWrapper):
    ...
    def _build_pos_bank(self):
        ...
        if not hasattr(self, "_pos_bank"):
            from transformers import AutoModel
            from physioex.models.foundation._checkpoints import ensure_reve_positions
    ...
    def _preprocess(self, x: torch.Tensor) -> torch.Tensor:
        """(B, C, T) -> (B, C', T) with DC removal + zscore + clip."""
        x = mean_center(x)
        x = zscore_clip(x, sigma_clip=_SIGMA_CLIP)
        return x
