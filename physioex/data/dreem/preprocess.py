...
processed_path = [ str(Path.home()) + "/dreem/raw/", str(Path.home()) + "/dreem/xsleepnet/"]
versions = ["dodo", "dodh"]
...
try:
    found = ( str( dirhash( utl.BASE_DIRECTORY_H5, "md5", jobs=os.cpu_count())) == utl.DATASET_HASH)
except:
    found = False
...
for data_path, version in zip(data_paths, versions):
    ...
    for f_name in tqdm(files):
        ...
        # raw
        fp =  np.memmap(f'{processed_path[0]}/{version}/C3-M2_{subject_id}.dat', dtype='float32', mode='w+', shape=C3.shape)
        fp[:] = C3[:]
        fp.flush()
        del fp

        fp = np.memmap(f'{processed_path[0]}/{version}/EOG_{subject_id}.dat', dtype='float32', mode='w+', shape=EOG.shape)
        fp[:] = EOG[:]
        fp.flush()
        del fp

        fp = np.memmap(f'{processed_path[0]}/{version}/EMG_{subject_id}.dat', dtype='float32', mode='w+', shape=EMG.shape)
        fp[:] = EMG[:]
        fp.flush()
        del fp

        fp = np.memmap(f'{processed_path[0]}/{version}/y_{subject_id}.dat', dtype='int16', mode='w+', shape=y.shape)
        fp[:] = y[:]
        fp.flush()
        del fp
        ...
        # raw
        fp =  np.memmap(f'{processed_path[1]}/{version}/C3-M2_{subject_id}.dat', dtype='float32', mode='w+', shape=C3.shape)
        fp[:] = C3[:]
        fp.flush()
        del fp
        

        fp = np.memmap(f'{processed_path[1]}/{version}/EOG_{subject_id}.dat', dtype='float32', mode='w+', shape=EOG.shape)
        fp[:] = EOG[:]
        fp.flush()
        del fp

        fp = np.memmap(f'{processed_path[1]}/{version}/EMG_{subject_id}.dat', dtype='float32', mode='w+', shape=EMG.shape)
        fp[:] = EMG[:]
        fp.flush()
        del fp

        fp = np.memmap(f'{processed_path[1]}/{version}/y_{subject_id}.dat', dtype='int16', mode='w+', shape=y.shape)
        fp[:] = y[:]
        fp.flush()
        del fp
