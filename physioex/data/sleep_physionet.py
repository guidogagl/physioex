...
class SleepPhysionet(PhysioExDataset):
    def __init__(self,  
                ...
                preprocessors : List[Preprocessor] = [
                    Preprocessor(lambda data: np.multiply(data, 1e6), apply_on_array=True),  # Convert from V to uV
        ...
        dataset = SP(subject_ids=config["subjects"], recording_ids=[1, 2], crop_wake_mins=30)
    ...
    def split(self, fold : int = 0):
        ...
        train_set, valid_set, test_set = splits["train"], splits["valid"], splits["test"]
