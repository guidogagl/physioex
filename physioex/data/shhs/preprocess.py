...
output_dirs = { "raw" : str(Path().home()) + "/shhs/raw/", "xsleepnet":  str(Path().home()) + "/shhs/xsleepnet/" }
...
def process_files(input_dir, output_dir_raw, output_dir_preprocessed, csv_path):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith(".mat"):
            subject_id, modality = filename.split('_')[0][1:], filename.split('_')[1].split('.')[0]
            ...
            raw_memmap = np.memmap(os.path.join(output_dir_raw, new_filename),  dtype='float32', mode='w+', shape=raw_data.shape)
            raw_memmap[:] = raw_data[:]
            raw_memmap.flush()
            del raw_memmap

            # Create memmap for preprocessed data
            preprocessed_memmap = np.memmap(os.path.join(output_dir_preprocessed, new_filename), dtype='float32', mode='w+', shape=preprocessed_data.shape)
            ...
            # Append to the list
            subject_samples.append([subject_id, modality, raw_data.shape[0], raw_mean, raw_std, preprocessed_mean, preprocessed_std])

    # Create a DataFrame and save to csv
    df = pd.DataFrame(subject_samples, columns=['Subject_ID', 'Modality', 'Samples', 'Raw_Mean', 'Raw_Std', 'Preprocessed_Mean', 'Preprocessed_Std'])
    df.to_csv(csv_path, index=False)
    
if __name__ == "__main__":
    
    # check if the output_dirs exists and create them if not
    for key in output_dirs.keys(): Path( output_dirs[ key ] ).mkdir(parents=True, exist_ok=True)