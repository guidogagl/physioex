...
def build_index( nums_windows, subjects_ids, sequence_length ):
    ...
    for i, (num_windows, subject_id) in enumerate(zip(nums_windows, subjects_ids)):
            ...
            if subject_id > np.iinfo(np.uint16).max:
                raise ValueError(f"subject_id {subject_id} exceeds the maximum value for np.uint16")
    
            # Check if the relative_id can be stored in a uint16
            if num_windows - sequence_length + 1 > np.iinfo(np.uint16).max:
                raise ValueError(f"Relative index {num_windows - sequence_length + 1} exceeds the maximum value for np.uint16")
        
            subject_idx[ start_index : start_index + num_windows - sequence_length + 1 ] = subject_id
            relative_idx[ start_index : start_index + num_windows - sequence_length + 1 ] = np.arange( num_windows - sequence_length + 1 )
