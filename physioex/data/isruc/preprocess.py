...
def get_labels(filepath):
    ...
    labels = np.array([int(label.strip()) for label in labels if label.strip().isdigit()]).astype(int)
    return labels
...
def read_edf(file_path):
    ...
    try: 
        ...
        try:
            ...
            logger.warning(f"No valid channels found for {filepath} available are {f.getSignalLabels()}")    
            return None, None
...
def save_memmaps(path, signal, labels, subject_id):
    ...
    for i,  modality  in enumerate(["EEG"]):
        ...
        fp = np.memmap(
            file_path, dtype="float32", mode="w+", shape= signal[i].shape
        )
        fp[:] = signal[i][:]
        fp.flush()
        del fp
...
# chmod 755 -R 
chmod_recursive(dl_dir, stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
...
sub1_desc = pd.read_excel( "http://dataset.isr.uc.pt/ISRUC_Sleep/Details/Details_subgroup_I_Submission.xlsx" )
sub3_desc = pd.read_excel( "http://dataset.isr.uc.pt/ISRUC_Sleep/Details/Details_subgroup_III_Submission.xlsx" )
...
table = table[ table["subject_id" not in discarded_subjects ] ]
