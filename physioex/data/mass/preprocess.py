...
def process_file(filename, id_dictionary, output_dir_raw, output_dir_preprocessed):
    ...
    if ( preprocessed_data > np.finfo(np.float32).max ).any():
        preprocessed_data[ preprocessed_data > np.finfo(np.float32).max ] = np.finfo(np.float32).max
    ...
    if ( preprocessed_data < np.finfo(np.float32).min ).any():
        preprocessed_data[ preprocessed_data < np.finfo(np.float32).min ] = np.finfo(np.float32).min
    ...
    assert not np.isinf( raw_data ).any(), f"Raw data has infinities for subject {subject_id} and modality {modality}"
    assert not np.isinf( preprocessed_data ).any(), f"Preprocessed data has infinities for subject {subject_id} and modality {modality}"
...
def process_files(input_dir, output_dir_raw, output_dir_preprocessed, csv_path):
    ...
    files_ids = [ filename[:-8] for filename in filenames if filename.endswith(".mat") and filename.startswith("SS") ]
    files_ids = list( set( files_ids ) )
    
    file_to_id = { filename: idx for idx, filename in enumerate( files_ids ) }
    
    results = Parallel(n_jobs=-1)(
        delayed(process_file)(filename, file_to_id, output_dir_raw, output_dir_preprocessed)
