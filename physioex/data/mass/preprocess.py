...
def process_file(filename, output_dir_raw, output_dir_preprocessed):
    
    modality = filename.split("_")[2].split(".")[0]

    # get the subject id as the indx of the file in the filelist with same modality
    
    filelist = [ file for file in os.listdir(input_dir) if file.endswith( modality + ".mat" ) ]
    subject_id = filelist.index( filename )    
...
def process_files(input_dir, output_dir_raw, output_dir_preprocessed, csv_path):
    ...
    results = Parallel(n_jobs=-1)(
        delayed(process_file)(filename, output_dir_raw, output_dir_preprocessed)
