...
class Svuh(PhysioExDataset):
    def __init__(
        ...
        for pick in picks:
            assert pick in AVAILABLE_PICKS, "pick should be one of 'EEG, 'EOG', 'EMG', 'ECG'"

        selected_channels = np.array( [ AVAILABLE_PICKS.index(pick) for pick in picks ] ).astype(int)
        input_shape = [4, 3000] if preprocessing == "raw" else [4, 29, 129] 
        
        super().__init__(
            dataset_folder = os.path.join( get_data_folder(), "svuh"),
