...
for key in tqdm(windows_dataset.keys()):
    ...
    # raw
    fp =  np.memmap(f'{raw_folder}/Fpz-Cz_{key}.dat', dtype='float32', mode='w+', shape=EEG.shape)
    fp[:] = EEG[:]
    fp.flush()
    del fp
    
    fp = np.memmap(f'{raw_folder}/EOG_{key}.dat', dtype='float32', mode='w+', shape=EOG.shape)
    fp[:] = EOG[:]
    fp.flush()
    del fp
    
    fp = np.memmap(f'{raw_folder}/EMG_{key}.dat', dtype='float32', mode='w+', shape=EMG.shape)
    fp[:] = EMG[:]
    fp.flush()
    del fp
    
    # xsleepnet
    fp = np.memmap(f'{xsleepnet_folder}/Fpz-Cz_{key}.dat', dtype='float32', mode='w+', shape=EEG_tf.shape)
    fp[:] = EEG_tf[:]
    fp.flush()
    del fp
    
    fp = np.memmap(f'{xsleepnet_folder}/EOG_{key}.dat', dtype='float32', mode='w+', shape=EOG_tf.shape)
    fp[:] = EOG_tf[:]
    fp.flush()
    del fp
    
    fp = np.memmap(f'{xsleepnet_folder}/EMG_{key}.dat', dtype='float32', mode='w+', shape=EMG_tf.shape)
    fp[:] = EMG_tf[:]
    fp.flush()
    del fp
    ...
    fp = np.memmap(f'{xsleepnet_folder}/y_{key}.dat', dtype='int16', mode='w+', shape=y.shape)
    fp[:] = y[:]
    fp.flush()
    del fp
...
table = pd.DataFrame(
    table, columns=["subject_id", "age", "sex", "num_samples"]
)
