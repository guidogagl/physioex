...
def write_subjet_data(h5_file, raw_signal, xsleepnet_signal, labels, subject_name):
    ...
    for i, channel in enumerate(channels):
        ...
        xsleepnet.create_dataset(
            channel,
            data=xsleepnet_signal[:, i],
            chunks=xsleepnet_chunk_shape,
            compression="gzip",
            compression_opts=9,
        )
...
def convert_to_h5py(
    ...
    with h5py.File(os.path.join(dataset_folder, "hpc_dataset.h5"), "w") as h5_file:
        ...
        for subject_name in tqdm(labels_files, desc="Subject fetching"):
            ...
            subject_index += np.arange(
                len(subject_index), len(subject_index) + labels.shape[0]
            ).tolist()
        subject_index = np.array(subject_index).astype(np.uint32)
        h5_file.create_dataset(
            "subject_index", data=subject_index, compression="gzip", compression_opts=9
        )
    ...
    if remote_folder and remote_user and remote_address:
        ...
        try:
            with sftp.file(
                os.path.join(remote_folder, "hpc_dataset.h5"), "wb"
            ) as remote_file:
            ...
            if os.path.exists(table_path):
                with sftp.file(
                    os.path.join(remote_folder, "table.csv"), "wb"
                ) as remote_file:
                    ...
                    with tqdm(
                        ...
                        unit="B",
                        unit_scale=True,
                        desc="Trasferimento file table.csv",
                    ) as pbar:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, help="Percorso della cartella del dataset"
    )
    parser.add_argument(
        "--remote_folder", type=str, default=None, help="Percorso della cartella remota"
    )
    parser.add_argument(
        "--remote_user",
        type=str,
        default=None,
        help="Nome utente per la connessione SSH",
    )
    parser.add_argument(
        "--remote_address", type=str, default=None, help="Indirizzo del server remoto"
    )
    ...
    convert_to_h5py(
        dataset_folder, args.remote_folder, args.remote_user, args.remote_address
    )
