...
def read_sleepdata_annotation(filename):
    ...
    assert (starts[-1] + durations[-1]) / 30 == len(stages), "Error: mismatch in number of epochs in annotations file"
    ...
    for channel in possible_channels:
        if isinstance(channel, tuple):
            if all([ch in av_channels for ch in channel]):
                return ( available_channels[av_channels.index(channel[0])], available_channels[av_channels.index(channel[1])])
        else:
