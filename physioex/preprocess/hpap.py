...
class HPAPPreprocessor(Preprocessor):
    ...
    def get_subjects_records(self) -> List[str]:
        ...
        for root, dirs, files in os.walk(edf_dir):
            for f in files:
                ...
                xml_path = os.path.join( records_dir, "annotations-events-nsrr", "lab", lab, f"{basename}-nsrr.xml" ) 
    ...
    def read_subject_record(self, record: str) -> Tuple[np.array, np.array]:
        ...
        n = subjects_records.index(record)
        
        raw_path = os.path.join(self.dataset_folder, "raw")
        xsleepnet_path = os.path.join(self.dataset_folder, "xsleepnet")
        
        return process_sleepdata_file(n, edf_path, xml_path, raw_path, xsleepnet_path)
...
if __name__ == "__main__":
    ...
    p = HPAPPreprocessor(data_folder="/home/guidogl/physioex-data/")
