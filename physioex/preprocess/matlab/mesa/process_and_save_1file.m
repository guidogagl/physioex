function [ret] = process_and_save_1file(filename, n)
    ...
    [eeg, ori_fs] = read_shhs_edfrecords([edf_path, filename], {'EEG1'});
    ...
    emg = filtfilt(b_band,1,emg);
        % Calcola il numero di epoch attesi
    
    expected_epochs = length(eeg) / (epoch_second * fs);

    % Calcola il numero di campioni per epoch
    num_samples = epoch_second * fs;
    
    % Verifica che le lunghezze delle variabili siano sufficienti
    assert(length(eeg) >= num_samples, 'Errore: la lunghezza di EEG è inferiore al numero di campioni per epoch.');
    assert(length(eog) >= num_samples, 'Errore: la lunghezza di EOG è inferiore al numero di campioni per epoch.');
    assert(length(emg) >= num_samples, 'Errore: la lunghezza di EMG è inferiore al numero di campioni per epoch.');
    
    % Verifica che la lunghezza dei segnali sia un multiplo del numero di campioni per epoch
    num_epochs = floor(length(eeg) / num_samples);
    eeg = eeg(1:num_epochs * num_samples);
    eog = eog(1:num_epochs * num_samples);
    emg = emg(1:num_epochs * num_samples);

    % Suddividi i segnali in epoch
    eeg_epochs = buffer(eeg, num_samples);
    eeg_epochs = eeg_epochs';
    eog_epochs = buffer(eog, num_samples);
    eog_epochs = eog_epochs';
    emg_epochs = buffer(emg, num_samples);
    emg_epochs = emg_epochs';

    fprintf('Numero di epoch attesi: %d', expected_epochs)
    fprintf('Numero di epoch calcolati: %d', size(eeg_epochs, 1))
    fprintf('Numero di stage calcolati: %d', numel(stages))

    for i = min(stages):max(stages)
        count = sum(stages == i);
        fprintf('Numero di elementi per stage %d: %d\n', i, count);
    end
