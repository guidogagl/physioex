...
def plot(
    ...
    if bands is not None:
        assert F == len(bands), "Number of bands must match the number of frequency bands"
        assert all( len(band) == 2 for band in bands), "Each band must be a tuple with 2 elements"
    ...
    fig, axs = plt.subplots(C * 2, S, figsize=(25, 5), sharex='col', sharey='row')
    ...
    for c in range(C):
        ...
        for s in range(S):
            ...
            if s != 0:  # Only remove y labels if s is not 0
            ...
            if s == 0 and bands is not None:  # Only remove y labels if s is not 0
                axs[ attr_row, s ].set_yticklabels( [ band[0] for band in bands] )
            else:
