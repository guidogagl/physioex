...
class ARIExplainer(PhysioExplainer):
    def __init__(self, 
        ...
        assert os.path.isdir(ckp_path), "ckp_path must be a valid directory containing at least one checkpoint"
        ...
        for elem in os.scandir(ckp_path):
            if elem.is_file() and elem.name.endswith(".ckpt"):
                try:
                    ...
                    logger.warning("Could not parse fold number from checkpoint name: %s. Skipping..." % elem.name)
                    continue
    ...
    def compute_ari(self, fold : int = 0, plot_pca : bool = False, plot_kmeans : bool = False):
        ...
        projections, y_true, y_pred = compute_projections(model, datamodule.train_dataloader(), model_device)
        ...
        if plot_pca:
            ...
            sns.scatterplot(x=components[:,0], y=components[:, 1],  hue=y_pred, palette='Set1')
            plt.xlabel("PC1")
            plt.ylabel("PC2")
        ...
        if plot_kmeans:
            ...
            kmeans = KMeans(n_clusters= self.module_config["n_classes"], random_state=0).fit(projections) 
            ...
            logger.info("JOB:%d-Plotting PCA" % fold)
            plt.figure(figsize=(10, 10))
            sns.scatterplot(x=components[:,0], y=components[:, 1], hue=kmeans.labels_, palette='Set1')
            plt.xlabel("PC1")
            plt.ylabel("PC2")
            plt.title(self.model_name + " PCA")
            plt.tight_layout()
        ...
        sns.lineplot(data= results, x="Number of clusters (K)", y = "Adjusted Rand Index",  marker='o')
    ...
    def explain(self, save_csv : bool = False, plot_pca : bool = False, plot_kmeans : bool = False, n_jobs : int = 10):
        results = []

        # Esegui compute_ari per ogni checkpoint in parallelo
        results = Parallel(n_jobs=n_jobs)(delayed(self.compute_ari)(int(fold), plot_pca, plot_kmeans) for fold in self.checkpoints.keys())
        ...
        sns.relplot(
            data=df, kind="line",
            x="Number of clusters (K)", y="Adjusted Rand Index", hue="Fold"
        )
        plt.title(self.model_name + " ARI")
        plt.tight_layout()
