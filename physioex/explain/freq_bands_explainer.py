...
class FreqBandsExplainer(PhysioExplainer):
    ...
    def compute_band_time_importance(self, bands : List[List[float]], band_names : List[List[str]], model : torch.nn.Module, dataloader : DataLoader, model_device : torch.device, sampling_rate: int = 100, class_names : list[str] = ["Wake", "N1", "N2", "DS", "REM"], average_type : int = 0, target_band : int = 0, target_class : int = 0):
        ...
        for batch in dataloader:
            ...
            for i in range(batch_size):           
                ...
                new_dataloader = DataLoader(
                    TensorDataset(nd_inputs, nd_y_true_batch),
                    batch_size=dataloader.batch_size,
                    shuffle=False,
                    num_workers=dataloader.num_workers,
                    collate_fn=dataloader.collate_fn,
...
#        plot_matrix = target_band_time_importance[0]
    ...
    def compute_band_importance(self, bands : List[List[float]], band_names : List[str], fold : int = 0, plot_pred : bool = False, plot_true : bool = False, save_csv : bool = False):
        ...
        target_band_time_importance = self.compute_band_time_importance(bands, band_names, model, datamodule.train_dataloader(), model_device, self.sampling_rate, self.class_name, 0, 3, 0)
