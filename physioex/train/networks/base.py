    ...
    def __init__(self, nn: nn.Module, config: Dict):
        ...
        if self.n_classes > 1:
            ...
            self.cm = tm.ConfusionMatrix(
                task="multiclass", num_classes=config["n_classes"], normalize=None
            )
    ...
    def compute_loss(
        ...
        if self.n_classes > 1:
            ...
            self.log(
                ...
                prog_bar=True,
                sync_dist=True,
                on_epoch=False if log == "train" else True,
            )
            self.log(
                f"{log}/acc",
                self.wacc(outputs, targets),
                prog_bar=True,
                sync_dist=True,
                on_epoch=False if log == "train" else True,
            )
            ...
            self.log(
                f"{log}/r2", self.r2(outputs, targets), prog_bar=True, sync_dist=True
            )
            self.log(
                f"{log}/mae", self.mae(outputs, targets), prog_bar=True, sync_dist=True
            )
            self.log(
                f"{log}/mse", self.mse(outputs, targets), prog_bar=True, sync_dist=True
            )

        if log_metrics and self.n_classes > 1:
            self.log(
                f"{log}/ck",
                self.ck(outputs, targets),
                sync_dist=True,
                on_epoch=False if log == "train" else True,
            )
            self.log(
                f"{log}/pr",
                self.pr(outputs, targets),
                sync_dist=True,
                on_epoch=False if log == "train" else True,
            )
            self.log(
                f"{log}/rc",
                self.rc(outputs, targets),
                sync_dist=True,
                on_epoch=False if log == "train" else True,
            )
            self.log(
                f"{log}/macc",
                self.macc(outputs, targets),
                sync_dist=True,
                on_epoch=False if log == "train" else True,
            )
            self.log(
                f"{log}/mf1",
                self.mf1(outputs, targets),
                sync_dist=True,
                on_epoch=False if log == "train" else True,
            )
