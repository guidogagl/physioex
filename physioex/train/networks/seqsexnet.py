...
from physioex.train.networks.seqsleepnet import EpochEncoder, SequenceEncoder, AttentionLayer
...
class WholeNightAgeNet(SleepModule):
    def __init__(self, module_config=module_config):
        ...
        super(WholeNightAgeNet, self).__init__(WholeNightNet(module_config=module_config), module_config)
    ...
    def log_correlation( self, outputs, targets, log: str = "train"):
        ...
        sns.scatterplot(x=targets.view(-1).cpu().numpy(), y=outputs.view(-1).cpu().numpy())
        plt.xlabel('True Age')
