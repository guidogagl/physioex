"""Evaluate sleeptransformer-phan on all supported datasets.

Downloads the pretrained model via ``load_from_pretrained("sleeptransformer-phan")``,
then evaluates it on the test split of every available dataset using per-subject
voting (sliding window L=21).

Usage:
    python examples/pretrained/sleeptransformer-phan/test_pretrained.py --gpu_id 0
    python examples/pretrained/sleeptransformer-phan/test_pretrained.py --gpu_id 0 --datasets shhs sleepedf
    python examples/pretrained/sleeptransformer-phan/test_pretrained.py --gpu_id 0 --upload
"""
import argparse
import json
import os
...
def print_summary_table(all_results):
    ...
    print("\n" + "=" * 60)
    print("SleepTransformer-Phan — Per-Dataset Evaluation Summary")
    print("=" * 60)
    print(header)
    print("-" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate sleeptransformer-phan on all supported datasets"
    )
    parser.add_argument(
    ...
    print("Loading pretrained SleepTransformer from HuggingFace...")
