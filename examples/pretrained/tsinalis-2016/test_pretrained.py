"""Evaluate tsinalis-2016 on all supported datasets.

Downloads the pretrained model via ``load_from_pretrained("tsinalis-2016")``,
then evaluates it on the test split of every available dataset.
Datasets whose data is not found locally are skipped gracefully.

Note: TsinalisCNN takes (B, 5, 1, 3000) and outputs (B, 1, 5) — it predicts
the central epoch only.  Evaluation uses Trainer.evaluate (not voting_evaluate)
since the model has no sequence-to-sequence output.

Usage:
    python examples/pretrained/tsinalis-2016/test_pretrained.py --gpu_id 0
    python examples/pretrained/tsinalis-2016/test_pretrained.py --gpu_id 0 --datasets sleepedf hmc
    python examples/pretrained/tsinalis-2016/test_pretrained.py --gpu_id 0 --upload
"""
import argparse
import json
import os
...
def print_summary_table(all_results):
    ...
    print("\n" + "=" * 60)
    print("Tsinalis-2016 — Per-Dataset Evaluation Summary")
    print("=" * 60)
    print(header)
    print("-" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate tsinalis-2016 on all supported datasets"
    )
    parser.add_argument(
    ...
    # ── Load model ───────────────────────────────────────────────────
    print("Loading pretrained TsinalisCNN from HuggingFace...")
