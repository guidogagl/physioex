"""Extract per-epoch embeddings for tsinalis-2016.

Uses centered-window extraction: for each epoch, builds a window of
L=5 epochs centered on it and extracts the CNN feature vector.
The model concatenates 5 epochs into one signal, so inter-epoch
context is preserved.

Usage:
    python examples/pretrained/tsinalis-2016/extract_embeddings.py --gpu_id 0
    python examples/pretrained/tsinalis-2016/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models.tsinalis import TsinalisCNN, extract_embeddings
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract embeddings for tsinalis-2016"
    )
