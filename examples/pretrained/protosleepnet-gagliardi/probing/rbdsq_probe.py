"""RBDSQ binary classification with LOSO cross-validation.

Classifies RBD- (RBDSQ < 5) vs RBD+ (RBDSQ >= 5) using codebook prototype
permanence features (proportion, bout mean, bout std per prototype).

Usage:
    python rbdsq_probe.py \
...
def load_subjects(emb_dirs, codebook):
    ...
    for emb_dir in emb_dirs:
        for mp in sorted(glob.glob(os.path.join(emb_dir, "*_metadata.json"))):
            ...
            label = 1 if rbdsq >= RBDSQ_CUTOFF else 0
...
def main():
    parser = argparse.ArgumentParser(
        description="RBDSQ binary classification (LOSO)"
    )
    ...
    # Save
    output_dir = os.path.join(args.output_dir, args.source_name, "rbdsq")
