...
that prototype. Saves the raw spectrogram epochs for downstream
...
from utils import (
    ...
    load_codebook, load_train_embeddings,
...
def main():
    ...
    # ── Phase 1: Embedding scan ──────────────────────────────────────
    print(f"Phase 1: Loading train embeddings for {args.backbone}...")
    codebook = load_codebook(args.backbone, m=args.m)
    subjects = load_train_embeddings(args.backbone)
    ...
    needed = defaultdict(set)  # subject_id -> set of epoch_idx
    for k in range(M):
    ...
    # ── Phase 2: Raw epoch retrieval ─────────────────────────────────
    print("Phase 2: Loading raw epochs from dataset...")
    _, train_loader = build_train_loader(args.backbone)

    # Stream through subjects, extract needed epochs
    epoch_store = {}  # (subject_id, epoch_idx) -> (C, T, F) numpy
    for batch in tqdm(train_loader, desc="Loading subjects"):
    ...
    # ── Phase 3: Save per-prototype results ──────────────────────────
