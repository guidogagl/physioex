"""Evaluate tinysleepnet-supratak on all supported datasets.

Downloads the pretrained model via ``load_from_pretrained("tinysleepnet-supratak")``,
...
Usage:
    python examples/pretrained/tinysleepnet-supratak/test_pretrained.py --gpu_id 0
    python examples/pretrained/tinysleepnet-supratak/test_pretrained.py --gpu_id 0 --datasets sleepedf hmc
    python examples/pretrained/tinysleepnet-supratak/test_pretrained.py --gpu_id 0 --upload
"""
import argparse
import json
import os
...
def print_summary_table(all_results):
    ...
    print("\n" + "=" * 60)
    print("TinySleepNet-Supratak — Per-Dataset Evaluation Summary")
    print("=" * 60)
    print(header)
    print("-" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate tinysleepnet-supratak on all supported datasets"
    )
    parser.add_argument(
    ...
    # ── Load model ───────────────────────────────────────────────────
    print("Loading pretrained TinySleepNet from HuggingFace...")
