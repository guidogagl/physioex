"""Extract contextualized embeddings for cbramod.

Loads the pretrained CBraMod encoder, then extracts per-epoch
...
Usage:
    python examples/foundation/cbramod/extract_embeddings.py --gpu_id 0
    python examples/foundation/cbramod/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models import CBraModEncoder, extract_embeddings, linear_probe
...
def main():
    ...
    dataset = CBraModEncoder.get_dataset(args.datasets[0] if args.datasets else "hmc")
