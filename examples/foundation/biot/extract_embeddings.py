"""Extract contextualized embeddings for biot.

Loads the pretrained BIOT encoder, then extracts per-epoch
...
Usage:
    python examples/foundation/biot/extract_embeddings.py --gpu_id 0
    python examples/foundation/biot/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models import BIOTEncoder, extract_embeddings, linear_probe
...
def main():
    ...
    dataset = BIOTEncoder.get_dataset(args.datasets[0] if args.datasets else "hmc")
    model = BIOTEncoder(in_chan=len(dataset.channels))
