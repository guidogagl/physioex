"""Extract contextualized embeddings for tfc.

Loads the pretrained TFC encoder, then extracts per-epoch
...
Usage:
    python examples/foundation/tfc/extract_embeddings.py --gpu_id 0
    python examples/foundation/tfc/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models import TFCEncoder, extract_embeddings, linear_probe
...
def main():
    ...
    dataset = TFCEncoder.get_dataset(args.datasets[0] if args.datasets else "hmc")
    model = TFCEncoder(in_chan=len(dataset.channels))
