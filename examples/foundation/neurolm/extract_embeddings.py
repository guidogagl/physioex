"""Extract contextualized embeddings for neurolm.

Loads the pretrained NeuroLM encoder, then extracts per-epoch
...
Usage:
    python examples/foundation/neurolm/extract_embeddings.py --gpu_id 0
    python examples/foundation/neurolm/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models import NeuroLMEncoder, extract_embeddings, linear_probe
...
def main():
    ...
    dataset = NeuroLMEncoder.get_dataset(args.datasets[0] if args.datasets else "hmc")
    model = NeuroLMEncoder(in_chan=len(dataset.channels))
