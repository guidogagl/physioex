"""Extract contextualized embeddings for labram.

Loads the pretrained LaBraM encoder, then extracts per-epoch
...
Usage:
    python examples/foundation/labram/extract_embeddings.py --gpu_id 0
    python examples/foundation/labram/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models import LaBraMEncoder, extract_embeddings, linear_probe
...
def main():
    ...
    dataset = LaBraMEncoder.get_dataset(args.datasets[0] if args.datasets else "hmc")
    model = LaBraMEncoder(in_chan=len(dataset.channels))
