"""Extract contextualized embeddings for bendr.

Loads the pretrained BENDR encoder, then extracts per-epoch
...
Usage:
    python examples/foundation/bendr/extract_embeddings.py --gpu_id 0
    python examples/foundation/bendr/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models import BENDREncoder, extract_embeddings, linear_probe
...
def main():
    ...
    dataset = BENDREncoder.get_dataset(args.datasets[0] if args.datasets else "hmc")
    model = BENDREncoder(in_chan=len(dataset.channels))
