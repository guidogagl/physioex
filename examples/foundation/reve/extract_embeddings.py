"""Extract contextualized embeddings for reve.

Loads the pretrained REVE encoder, then extracts per-epoch
...
Usage:
    python examples/foundation/reve/extract_embeddings.py --gpu_id 0
    python examples/foundation/reve/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models import REVEEncoder, extract_embeddings, linear_probe
...
def main():
    ...
    dataset = REVEEncoder.get_dataset(args.datasets[0] if args.datasets else "hmc")
    model = REVEEncoder(in_chan=len(dataset.channels))
