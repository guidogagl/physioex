"""Extract contextualized embeddings for sleepfm.

Loads the pretrained SleepFM encoder, then extracts per-epoch
...
Usage:
    python examples/foundation/sleepfm/extract_embeddings.py --gpu_id 0
    python examples/foundation/sleepfm/extract_embeddings.py --gpu_id 0 --datasets sleepedf hmc
"""
import argparse

from physioex.data.datasets import available_datasets, get_dataset
from physioex.models import SleepFMEncoder, extract_embeddings, linear_probe
...
def main():
    ...
    dataset = SleepFMEncoder.get_dataset(args.datasets[0] if args.datasets else "hmc")
    model = SleepFMEncoder(in_chan=len(dataset.channels))
